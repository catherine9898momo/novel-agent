...
- 开始新的实现任务前，运行 `npm run career -- status`。如果存在符合条件的待处理提交，调用项目 `career-capture` Skill，并在继续实现前询问用户。
- 成功 commit 后，在宣布任务完成前，为该提交调用项目 `career-capture` Skill。
- 未经用户明确确认，不得生成 Career 案例；生成后也不得自动提交案例文档。
- 仅修改 `career-prepare/novel-agent/**` 的提交，或主题以 `docs(career):` 开头的提交，不触发新的 Career Capture。
- Career Capture 是文档旁路，不得绕过、替代或改变 Novel Agent 故事创作主工作流。
- Career 代码、Hook、案例和计划只保留在 `codex/career-capture` 及其同名远端分支；确有主系统收益的内容必须拆成独立 commit，经用户确认后才能进入本地 `main`。
